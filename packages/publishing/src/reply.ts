...
import { replizActiveCredentials, replizReplyComment } from "./repliz";
...
async function replyYouTube(input: ReplyInput): Promise<ReplyResult> {
  ...
  // Guard: minimal salah satu harus ada (sama dgn assertion lama, tanpa `!`).
...
/**
 * Reply akun bridge Repliz: token platform disimpan Repliz, jadi balasan dikirim
 * lewat Comment API Repliz (POST /public/comment/{commentId}).
 * platformItemId = _id komentar Repliz (disimpan saat sync komentar bridge).
 */
async function replyRepliz(input: ReplyInput): Promise<ReplyResult> {
  if (!input.platformItemId) {
    throw new PublishError("no_platform_item", "Item tidak punya ID komentar Repliz.", false);
  }
  ...
  const replyId = await replizReplyComment(cred, input.platformItemId, input.content);
  return { replyId };
}
...
export async function sendReply(input: ReplyInput): Promise<ReplyResult> {
  // Akun bridge Repliz: routing reply lewat Comment API Repliz (sebelum dispatch
  // native — token platform tidak ada di sisi kita, Repliz yang menyimpannya).
  if (input.accountMetadata?.replizAccountId) {
    return replyRepliz(input);
  }
