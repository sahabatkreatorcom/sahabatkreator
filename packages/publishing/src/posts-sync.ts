...
import { type ReplizContent, replizActiveCredentials, replizListContent } from "./repliz";
...
export async function syncWorkspacePosts(
  ...
  // fetchExternalPosts dispatch ke fetchReplizPosts via metadata.replizAccountId.
  const syncable = accounts.filter(
    (a) =>
      POSTS_SYNC_PLATFORMS.has(a.platform) && (a.accessTokenEnc || a.metadata?.replizAccountId),
  );
...
/** Mapping jenis konten Repliz → mediaType kita (text/link → IMAGE, tanpa media) */
function mapReplizMediaType(type: string | undefined): ExternalPost["mediaType"] {
  switch (type) {
    case "video":
      return "VIDEO";
    case "reel":
...
/** Mapping satu konten Repliz → ExternalPost */
function mapReplizContent(c: ReplizContent): ExternalPost {
  const firstMedia = c.medias?.[0];
  return {
    externalId: c.id,
    caption: [c.title, c.description].filter(Boolean).join("\n").slice(0, 2000),
    mediaType: mapReplizMediaType(c.type),
...
/**
 * Fetch konten akun bridge via Repliz Content API (Gold+).
 * Dua pass: type "media" (post/reel/video) + "story"; masing-masing paginasi
 * cursor nextToken (docs: response menunjukkan urut createdAt DESCENDING —
 * berhenti saat menemukan konten lebih lama dari window `since`).
 */
async function fetchReplizPosts(
  account: SyncableAccount,
  since: Date,
): Promise<ExternalPost[] | { error: string }> {
  const cred = await replizActiveCredentials();
  if (!cred) return { error: "Bridge Repliz tidak aktif" };
  const accountId = account.metadata?.replizAccountId as string | undefined;
  if (!accountId) return { error: "Akun bridge tanpa replizAccountId" };

  const out: ExternalPost[] = [];
  for (const type of ["media", "story"] as const) {
    let nextToken: string | undefined;
    // Batas halaman untuk menghindari loop tak terhingga bila cursor macet
    for (let page = 0; page < 5; page++) {
      let res: { docs: ReplizContent[]; nextToken?: string };
      try {
        res = await replizListContent(cred, accountId, { nextToken, type });
      } catch (error) {
        return {
          error: `Repliz content: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
      let hitOld = false;
      for (const c of res.docs) {
        const publishedAt = c.createdAt ? new Date(c.createdAt) : null;
        if (publishedAt && publishedAt < since) {
          hitOld = true;
          continue;
        }
        out.push(mapReplizContent(c));
      }
      if (hitOld || !res.nextToken) break;
...
async function fetchExternalPosts(
  ...
  // Akun bridge Repliz: konten via Content API Repliz (token platform disimpan
  // Repliz; accessToken lokal tidak dipakai).
  if (account.metadata?.replizAccountId) {
    return fetchReplizPosts(account, since);
  }
...
async function syncAccountPosts(account: SyncableAccount, since: Date): Promise<PostSyncResult> {
  ...
  if (!account.accessTokenEnc && !account.metadata?.replizAccountId) {
    return { ...base, error: "Token tidak tersedia" };
  }
  ...
  // Akun bridge tidak punya token lokal (Repliz menyimpannya) → lewati refresh.
  if ("error" in fetched && isPermanentTokenError(fetched.error) && account.accessTokenEnc) {
