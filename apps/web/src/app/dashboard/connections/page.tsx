...
const REFRESH_TOKEN_PLATFORMS: Platform[] = [
  "YOUTUBE",
  "GOOGLE_BUSINESS",
  "TIKTOK",
  "PINTEREST",
...
function tokenStatus(
  ...
  if (!account.tokenExpiry)
    return { label: "Token tidak diketahui", tone: "none" };
  const expiry = new Date(account.tokenExpiry).getTime();
  const now = Date.now();
  if (expiry < now)
    return { label: "Token kedaluwarsa — hubungkan ulang", tone: "expired" };
  const daysLeft = (expiry - now) / 86_400_000;
  if (daysLeft < 7)
    return {
      label: `Token hampir kedaluwarsa (${Math.max(0, Math.floor(daysLeft))} hari)`,
      tone: "warn",
    };
  return {
    label: `Token valid hingga ${new Date(expiry).toLocaleDateString("id-ID", { timeZone: "Asia/Jakarta" })}`,
    tone: "ok",
  };
}
...
export default function ConnectionsPage() {
  ...
  const loadPendingChoices = async (pendingId: string, platform: Platform) => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(data.error || "Gagal memuat pilihan halaman.");
      ...
      setError(
        e instanceof Error ? e.message : "Gagal memuat pilihan halaman.",
      );
      router.replace("/connections");
    } finally {
  ...
  useEffect(() => {
    ...
    const needsRefresh = accounts.some(
      (a) =>
        a.lastRefreshError ||
        (a.tokenExpiry &&
          new Date(a.tokenExpiry).getTime() < Date.now() + 7 * 86_400_000),
    );
    ...
    fetch("/api/accounts/refresh")
      ...
      .then((data) => {
        if (data.results?.some((r: { refreshed: boolean }) => r.refreshed)) {
          setNotice(
            "Token yang hampir kedaluwarsa berhasil diperbarui otomatis.",
          );
        } else if (data.results?.some((r: { error?: string }) => r.error)) {
          const failed = data.results.find((r: { error?: string }) => r.error);
          setError(
            `Beberapa akun perlu dihubungkan ulang: ${failed?.error ?? "token tidak bisa diperbarui."}`,
          );
        }
        loadAccounts();
      })
    ...
    try {
      ...
      setNotice(
        data.refreshed
          ...
          : "Token masih valid, tidak perlu diperbarui.",
      );
      loadAccounts();
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              <p
                className={`text-2xl font-semibold ${accountsWithIssue > 0 ? "text-accent-amber" : ""}`}
              >
                {accountsWithIssue}
              </p>
              <p className="text-xs text-muted-foreground">
                Akun perlu perhatian (token / error)
              </p>
            </div>
          </div>
      ...
      <Dialog
        ...
        title="Detail akun"
        description={
          showInfoDialog ? `${PLATFORM_LABELS[showInfoDialog.platform]}` : ""
        }
      >
        {showInfoDialog && (
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <PlatformAvatar
                platform={showInfoDialog.platform}
                avatar={showInfoDialog.avatar}
            ...
            <div className="rounded-md bg-muted p-3 text-sm">
              <div className="flex justify-between">
                ...
                <span
                  className={TOKEN_TONE_STYLE[tokenStatus(showInfoDialog).tone]}
                >
              ...
              <div className="mt-1 flex justify-between">
                ...
                <span>
                  {new Date(showInfoDialog.createdAt).toLocaleDateString(
                    "id-ID",
                    {
                      day: "numeric",
                      month: "long",
            ...
            {pendingChoices?.pages.map((page) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium">
                    {pendingChoices.platform === "INSTAGRAM_PAGE"
                      ? page.accountName || page.pageName
                      : page.pageName}
                  </p>
                  {pendingChoices.platform === "INSTAGRAM_PAGE" &&
                    page.username && (
