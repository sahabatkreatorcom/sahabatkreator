...
function TrendChart({
  ...
  data: SeriesPoint[];
  color: string;
  metric: "followers" | "impressions" | "reach";
  label: string;
}) {
  const chartData = useMemo(
    () =>
      data.map((p) => ({
        ...p,
        dateLabel: new Date(p.date).toLocaleDateString("id-ID", {
          day: "2-digit",
          month: "short",
        }),
      })),
...
export default function AnalyticsPage() {
  ...
  const _latestFollowersChange =
    overview?.perPlatform.reduce((a, p) => a + (p.followersChange ?? 0), 0) ??
    0;
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Performa akun sosial workspace Anda.
          </p>
        </div>
      ...
      {liveOnlyPlatforms.length > 0 && (
        <p className="rounded-md bg-accent-amber/10 px-3 py-2 text-sm text-accent-amber">
          Platform {liveOnlyPlatforms.map((p) => PLATFORM_LABELS[p]).join(", ")}{" "}
          tidak menyimpan metrik karena kebijakan platform — data ditampilkan
          langsung (live) dari platform.
        </p>
      )}
      ...
      {loading && !overview ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <StatCard
              label="Pengikut"
              value={formatNumber(overview?.totals.followers ?? 0)}
            />
            <StatCard
              label="Tayangan"
              value={formatNumber(overview?.totals.impressions ?? 0)}
            />
            <StatCard
              label="Jangkauan"
              value={formatNumber(overview?.totals.reach ?? 0)}
            />
            <StatCard
              label="Post terbit"
              value={formatNumber(overview?.totals.postsPublished ?? 0)}
            />
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium">
                      {p.accountName ?? PLATFORM_LABELS[p.platform]}
                    </p>
            ...
            {liveOnlyPlatforms.map((p) => (
              ...
              >
                ...
                <p className="mt-3 text-xs text-muted-foreground">
                  Metrik tidak disimpan (kebijakan platform) — tampilkan
            ...
            {!loading &&
              overview?.perPlatform.length === 0 &&
              liveOnlyPlatforms.length === 0 && (
                <div className="col-span-full rounded-lg border border-border bg-card p-8 text-center text-sm text-muted-foreground">
                  Belum ada data. Hubungkan akun sosial lalu klik{" "}
                  <span className="font-medium">Sinkronkan</span>.
                </div>
              )}
          </div>
        </div>
