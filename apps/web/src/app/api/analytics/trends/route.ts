...
export const GET = withAuth(async (ctx, req: NextRequest) => {
  ...
  const _platform = searchParams.get("platform") ?? undefined;
  ...
  const topPosts = posts
    .filter(
      (pa) =>
        pa.post &&
        pa.post.organizationId === activeOrganizationId &&
        pa.post.status === "PUBLISHED",
    )
    .map((pa) => {
      const score =
        (pa.impressions ?? 0) * 0.5 +
        (pa.likes ?? 0) * 0.3 +
        (pa.comments ?? 0) * 0.2;
      return {
        postId: pa.postId,
  ...
  const platformAvg = [...byPlatform.entries()].map(([p, list]) => {
    const avg =
      list.length === 0
        ? 0
        : list.reduce((a, s) => a + s.engagementRate, 0) / list.length;
    return {
      platform: p,
