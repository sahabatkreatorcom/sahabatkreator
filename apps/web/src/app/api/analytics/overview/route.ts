...
export const GET = withAuth(async (ctx, req: NextRequest) => {
    ...
    for (const s of snapshots) {
        if (!platformMap.has(s.platform) || s.date > platformMap.get(s.platform)!.date) {
    ...
    const perPlatform = [...platformMap.entries()].map(([platform, s]) => ({
        ...
        accountAvatar: accounts.find((a) => a.platform === platform)?.avatar ?? null,
    }));
    ...
    const dateSet = new Set(snapshots.map((s) => s.date.toISOString().slice(0, 10)));
    const series = [...dateSet].sort().map((date) => {
        const daySnapshots = snapshots.filter((s) => s.date.toISOString().slice(0, 10) === date);
        return {
            date,
