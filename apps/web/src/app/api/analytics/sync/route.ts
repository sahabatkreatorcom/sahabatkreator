...
import { json, requireAuth, verifyCronSecret } from "@/lib/api";
...
export async function POST(req: NextRequest) {
  ...
  if (verifyCronSecret(req)) {
    ...
    if (!orgId)
      return json(
        { error: "x-organization-id wajib untuk panggilan cron." },
        { status: 400 },
      );
    ...
    if (orgId.toUpperCase() === "ALL") {
      ...
      const results = await Promise.all(
        organizations.map(async (org) => {
          try {
            const result = await syncOrganizationAnalytics(org.id);
