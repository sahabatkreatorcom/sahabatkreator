...
type TrendType = "all" | "google" | "music" | "video";

export function TrendsPage() {
  const [platform, setPlatform] = useState("instagram");
  const [type, setType] = useState<TrendType>("all");
  ...
  return (
    <div className="space-y-10">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-[var(--text-secondary)] text-sm">
            Tren Google, lagu, dan video populer Indonesia — manfaatkan untuk konten yang relevan
          </p>
        </div>
        {/* Filter tipe tren (bukan lagi platform sosial) */}
        <select
          value={type}
      ...
      <MediaTrendsSection
        showMusic={type === "all" || type === "music"}
        showVideo={type === "all" || type === "video"}
      />
        ...
        <section className="space-y-4">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-[var(--text-secondary)] text-sm">
                Kata kunci yang sedang naik hari ini — manfaatkan untuk konten yang relevan.
              </p>
            </div>
            {/* Pilihan platform untuk generate ide konten dari tren Google */}
            <select
              value={platform}
