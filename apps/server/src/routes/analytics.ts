  ...
  try {
    ...
    const rows = await db.execute(
      sql`select p.id as post_id,
          ...
          -- LEFT JOIN: post yang sudah published tetap muncul meski snapshot
          -- analytics belum ada (sync berjalan tiap 6 jam; platform tanpa akses
          -- baca metrik — mis. LinkedIn personal app "Share on LinkedIn" — tidak
          -- pernah menghasilkan baris). INNER JOIN lalu menyembunyikan post baru
          -- selama jam-jam pertama setelah tayang.
          left join lateral (
          ...
          left join lateral (
            select m.url, m.thumbnail_url, m.type
            from post_media pm
    ...
    return c.json({
      posts: (rows.rows as Record<string, unknown>[]).map((r) => {
        ...
        // Thumbnail prioritas: media table (post native via Compose) →
        // external_thumbnail_url (post impor dari platform: full_picture FB,
        // thumbnail_url IG, thumbnail Repliz). Video/reel hampir selalu jalur
        // kedua karena post_media hanya diisi oleh Compose.
        const settings = (r.platform_settings ?? {}) as { mediaType?: string; mediaUrl?: string };
        const settingsType = settings.mediaType?.toUpperCase();
        const mediaType = (r.media_type ?? settingsType ?? null) as string | null;
        const mediaUrl = (r.media_thumbnail_url ?? r.media_url ?? r.external_thumbnail_url) as
          | string
          | null;
        const isVideo =
          mediaType === "video" || ["VIDEO", "REEL", "STORY"].includes(settingsType ?? "");
        // URL video asli (media table utk post native, platform_settings.mediaUrl
  ...
  try {
    ...
    if (pinsRes.ok) {
      ...
      if (pinIds.length > 0) {
        ...
        if (anRes.ok) {
          const anData = (await anRes.json()) as Record<
            string,
          ...
          if (errBody.code !== 3022) {
            console.warn(
              `[pinterest-analytics] metrics fetch failed: ${JSON.stringify(errBody).slice(0, 150)}`,
            );
          }
        }
      }
