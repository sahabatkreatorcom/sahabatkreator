...
- Publisher saat ini mendukung IG (Feed/Reel/Story) & FB (Page). Platform lain → error UNSUPPORTED_PLATFORM yang jelas.
...
3. Publisher lanjutan: TikTok, YouTube, Pinterest, LinkedIn, Threads (copy dari socaliseit `lib/platforms/publishing/*` + `lib/platform-api/*`).
4. Auto-publish terjadwal: butuh worker/cron yang memindai `post.status=SCHEDULED AND scheduledAt<=now` lalu memanggil publish. Belum dibangun.
5. M4+: calendar, analytics, inbox, AI, billing UI, admin, ekstra (lihat todo sesi).
